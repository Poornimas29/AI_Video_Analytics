...
class GlobalSession:
    ...
    # Presence and entrance/exit status
    presence_status: str = "IN"  # "IN" (Inside) or "OUT" (Outside/Exited)
    last_camera_id: str = ""
    last_camera_type: str = "entrance"  # "entrance" or "inner"
    entrance_camera_id: Optional[str] = None
    ...
    prior_accumulated_seconds: float = 0.0
    prior_phone_seconds: float = 0.0
    prior_phone_count: int = 0
    ...
    def working_duration(self) -> float:
        """Returns cumulative on-site working duration (prior visits today + current visit).
        
        Excludes all gaps where the session was in "lost" or exited status.
        """
        return self.prior_accumulated_seconds + self.accumulated_working_seconds

    @property
    def current_visit_duration(self) -> float:
        """Returns duration of the current visit only."""
...
class GlobalSessionManager:
    ...
    def get_active_sessions(self) -> List[GlobalSession]:
        """Return all sessions that are currently active or inside the premises."""
        return [
            s for s in self.sessions.values()
            if s.status != "exited" and s.presence_status == "IN"
        ]
    ...
    def create_session(
        ...
        cam_type = settings.get_camera_type(camera_id)

        # Check for an existing non-exited session for this employee to prevent duplicate concurrent sessions
        for session in self.sessions.values():
            if session.employee_id == employee_id and session.status != "exited":
                # Rebind and continue existing session
                session.status = "tracking"
                session.presence_status = "IN"
                session.last_camera_id = camera_id
                session.last_camera_type = cam_type
        ...
        session = GlobalSession(
            ...
            last_camera_type=cam_type,
            entrance_camera_id=camera_id if settings.is_entrance_camera(camera_id) else None,
            camera_history=[{
                "cam_id": camera_id,
    ...
    def update_track(
        ...
        session.last_camera_id = camera_id
        session.last_camera_type = settings.get_camera_type(camera_id)
        ...
        for phone in phone_dets:
            p_box = getattr(phone, "bbox", phone)
            ph_x1, ph_y1, ph_x2, ph_y2 = p_box
        ...
        if phone_used:
            if session.phone_use_start is None:
                ...
                if overlap_time >= settings.PHONE_USAGE_CONFIRM_SECONDS:
                    if not session.phone_confirmed_use_active:
                        session.phone_confirmed_use_active = True
                        logger.warning(
                            "[SessionManager] Phone usage violation confirmed for Employee %s (%s)",
                            session.employee_id,
        ...
        if total_duration > 0.0:
            non_phone_time = max(0.0, total_duration - session.phone_use_duration)
    ...
    def bind_camera_track(
        ...
        if session.last_seen is not None and timestamp > session.last_seen:
            dt = (timestamp - session.last_seen).total_seconds()
            if 0.0 < dt < 300.0:
                session.accumulated_working_seconds += dt
        ...
        # Close any prior open entry for this camera if present
    ...
    def process_timeouts(self, timestamp: datetime) -> List[GlobalSession]:
        """Find and return sessions that have timed out from an Entrance/Exit camera.

        Entrance-gated logic:
        - If the employee was lost from an Entrance camera (CAM001/CAM002), they have exited the premises.
          We mark the session as 'exited', presence_status as 'OUT', and return in exited list.
        - If the employee was lost from an Inner camera (CAM003/CAM004), they are still inside!
          We keep presence_status as 'IN', transition status to 'inside_idle', and do NOT trigger exit.
        """
        exited = []
        for session in list(self.sessions.values()):
            if session.status == "lost":
                if timestamp - session.last_seen > self.lost_timeout:
                    ...
                    # Check whether the person disappeared from an Entrance camera or an Inner camera
                    is_entrance = settings.is_entrance_camera(session.last_camera_id)
                    
                    if is_entrance or not session.last_camera_id:
                        # Left from entrance camera -> Mark as EXITED (OUT)
                        session.status = "exited"
                        session.presence_status = "OUT"
                        session.exit_camera_id = session.last_camera_id or "CAM001"
                        exited.append(session)
                    else:
                        # Lost on an inner camera -> Still inside the building!
                        session.status = "inside_idle"
                        session.presence_status = "IN"
