...
class StreamManager:
    ...
    def stop(self) -> None:
        ...
        elif hasattr(self._stream, "cap") and self._stream.cap is not None:
            self._stream.cap.release()
        self._connected = False
        logger.info("StreamManager stopped for %s", self.camera_name)

    def set_speed(self, speed: float) -> None:
        """Set the playback speed on the underlying stream."""
        if hasattr(self._stream, "set_speed"):
    ...
    def _run(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                success, frame, frame_ts = self._stream.read()
                if success:
                    self._connected = True
                    self._fail_count = 0    # reset backoff on successful read
                    # Store latest frame with stream/video timestamp.
                    self.frame_buffer.add(frame, frame_ts)
                    h, w = frame.shape[:2]
                ...
                if self.is_eof():
                    ...
                    continue
                self._fail_count += 1
                is_file = getattr(self._stream, "_is_file", False)
                if is_file:
                    backoff = 0.1
                else:
                    backoff = min(self.reconnect_interval * (2 ** min(self._fail_count - 1, 2)), 30)
                logger.error("[%s] Stream read error (attempt %d, retry in %.1fs): %s",
                             self.camera_name, self._fail_count, backoff, exc)
                self._connected = False
                time.sleep(backoff)
        # Clean exit.
        self._connected = False
    ...
    def is_eof(self) -> bool:
        """True when the underlying file stream has reached end of file."""
        return getattr(self._stream, "is_eof", False)
