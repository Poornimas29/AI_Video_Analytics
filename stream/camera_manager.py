...
class CameraManager:
        ...
        if camera_cfg is not None:
            ...
            for cam in getattr(settings, "CAMERAS", []):
                if not cam.get("enabled", False):
                    continue
                cam_id = cam.get("id", "")
                cam_url = (cam.get("url") or "").strip()
                per_cam_playback = os.getenv(f"PLAYBACK_URL_{str(cam_id).upper()}", "").strip()

                if enable_playback:
                    # In playback mode: ONLY allow cameras with an explicit video URL
                    resolved = cam_url or per_cam_playback
                    if not resolved:
                        logger.info("[CameraManager] Camera %s has no video URL provided — omitting from active list.", cam_id)
                        continue
                    cam_copy = dict(cam)
                    cam_copy["url"] = resolved
                    self.camera_configs[cam_id] = cam_copy
                else:
                    self.camera_configs[cam_id] = cam
    ...
    def generate_rtsp_url(self, cam_id: any = None, channel: int = 1) -> str:
        """Generate default RTSP URL via settings."""
        from config import settings
        if isinstance(cam_id, int):
            channel = cam_id
            cam_id = ""
        return settings.build_default_rtsp_url(camera_id=cam_id or "", channel=channel)
    ...
    def set_speed(self, speed: float) -> None:
        """Dynamically update playback speed across all active camera streams."""
        for stream_mgr in self.streams.values():
            if hasattr(stream_mgr, "set_speed"):
                stream_mgr.set_speed(speed)
        logger.info("[CameraManager] Playback speed updated to %.1fx across all streams", speed)

    def all_streams_finished(self) -> bool:
        """Check if all video file streams have reached end of file."""
        if not self.streams:
            return False
        file_streams = [s for s in self.streams.values() if getattr(s._stream, "_is_file", False)]
        if not file_streams:
            return False
        return all(s.is_eof() for s in file_streams)
