...
class GridRenderer:
    ...
    def build_dashboard(
        ...
        speed: float = 1.0,
        current_ts: Optional[datetime.datetime] = None,
    ) -> np.ndarray:
        """
        ...
        header = self._build_header(grid.shape[1], len(camera_ids), connected, speed, current_ts)
    ...
    def _build_header(
        ...
        """
        Renders the top status bar with title, speed badge, camera online count, and clock.
        """
        ...
        # Center: Speed Badge
        if speed <= 0.0:
            speed_text = "⏸ PAUSED [Space to Resume]"
            speed_color = (60, 60, 240)  # Red/Amber
            bg_color = (25, 20, 50)
        else:
            speed_text = f"⚡ SPEED: {speed:.1f}x [+/- Keys]"
            speed_color = (0, 230, 255)  # Cyan/Gold
            bg_color = (35, 30, 20)

        badge_w = cv2.getTextSize(speed_text, cv2.FONT_HERSHEY_SIMPLEX, 0.52, 2)[0][0]
        badge_x = (width - badge_w) // 2
        cv2.rectangle(bar, (badge_x - 12, 10), (badge_x + badge_w + 12, self.HEADER_HEIGHT - 12), bg_color, -1)
        cv2.rectangle(bar, (badge_x - 12, 10), (badge_x + badge_w + 12, self.HEADER_HEIGHT - 12), speed_color, 1)
        cv2.putText(
            bar,
            speed_text,
            (badge_x, 32),
            cv2.FONT_HERSHEY_SIMPLEX, 0.52, speed_color, 2, cv2.LINE_AA,
        )

        # Right: camera count + current video timestamp
        online = sum(1 for v in connected.values() if v)
        display_dt = current_ts or datetime.datetime.now()
        ts = display_dt.strftime("%Y-%m-%d  %H:%M:%S")
        right_text = f"Cameras: {online}/{cam_count} Online    {ts}"
        text_w = cv2.getTextSize(right_text, cv2.FONT_HERSHEY_SIMPLEX, 0.52, 1)[0][0]
        cv2.putText(
            bar,
            right_text,
            (width - text_w - 16, 32),
