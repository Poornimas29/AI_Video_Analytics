...
class MonitoringService:
    ...
    def _write_json(self, cam_id: str, ts: datetime, sessions: list, unrecognized_tracks: list):
        ...
        for s in sessions:
            person_list.append({
                ...
                "bbox": s.current_bbox,
                "confidence": s.recognition_confidence,
                "is_using_phone": s.phone_confirmed_use_active,
                "pose_state": s.pose_state
            })
        for utrk in unrecognized_tracks:
            person_list.append({
                ...
                "name": "Unknown",
                "presence_status": "IN",
                "current_track_id": utrk["track_id"],
                "bbox": utrk["bbox"],
                "confidence": 0.0,
                "is_using_phone": False,
                "pose_state": utrk.get("pose_state")
            })
        out_data = {
            "camera_id": cam_id,
            "timestamp": ts.isoformat(),
            "detected_persons": person_list
        }
        filename = f"{cam_id}_{ts.strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def process_camera_frame(self, cam_id: str, frame: np.ndarray, ts: datetime):
        ...
        # ── STEP 2: Detect persons and objects on this frame ────────────────────
        all_detections = self.detection_manager.detect(frame)
        ...
        for _track in exited_tracks:
            self.identity_manager.release_track(_track["camera_id"], _track["track_id"])
        ...
        for trk in tracks:
            ...
            # Check if this track is already recognized / locked in identity_manager
            locked_emp_id = self.identity_manager.get_mapped_employee_id(cam_id, trk.track_id)
            if locked_emp_id and not track_mem.get("locked_status"):
                track_mem["locked_status"] = True
                track_mem["employee_id"] = locked_emp_id
                emp_obj = self.employee_manager.get_employee_by_id(locked_emp_id)
                track_mem["employee_name"] = emp_obj.name if emp_obj else locked_emp_id
            ...
            if is_new and feat is not None:
                ...
                for s in self.attendance_manager.sessions.values():
                    if s.status in ("lost", "inside_idle") and getattr(s, "reid_features", None) is not None:
        ...
        for trk in tracks:
            track_mem = self.track_memory_manager.get_track(cam_id, trk.track_id)
            # Track-ID Recognition Break: Once locked, break out and stop submitting!
        ...
        class LocalSessionProjection:
            def __init__(self, gs, camera_id):
                state = gs.visible_cameras.get(camera_id)
                if state is None:
                    state = CameraTrackState(
                        track_id=gs.current_track_id,
                        bbox=list(gs.current_bbox) if gs.current_bbox else [0, 0, 1, 1],
                ...
                self.working_duration = gs.working_duration
                self.current_visit_duration = getattr(gs, "current_visit_duration", gs.accumulated_working_seconds)
                self.presence_status = getattr(gs, "presence_status", "IN")
                self.camera_type = settings.get_camera_type(camera_id)
                self.phone_use_detected = getattr(state, "phone_use_detected", False)
    ...
    def shutdown(self, ts: datetime) -> None:
        """Force finalize all active and lost sessions and write final reports on system shutdown."""
        logger.info("[MonitoringService] Shutting down and finalising all active sessions...")
        # Stop background engines
        if hasattr(self, "async_recognizer"):
            self.async_recognizer.stop()
        if hasattr(self, "_pose_stop"):
            self._pose_stop.set()
        if hasattr(self, "_detect_stop"):
            self._detect_stop.set()
        
        # Force all known sessions to exit and write JSON output using their actual last_seen time
        for session in list(self.attendance_manager.sessions.values()):
            if session.status != "exited":
                actual_exit_time = session.last_seen if session.last_seen is not None else ts
                for entry in session.camera_history:
                    if entry.get("exit_time") is None:
                        entry["exit_time"] = actual_exit_time
                
                # Finalise phone usage if active
                if session.phone_use_start is not None:
                    duration = (actual_exit_time - session.phone_use_start).total_seconds()
                    if duration >= settings.PHONE_USAGE_CONFIRM_SECONDS:
                        session.phone_use_history.append({
                            "start": session.phone_use_start,
                            "end": actual_exit_time,
                            "duration": duration
                        })
                ...
                # Check if session was truly exited via an entrance door or was still inside when video ended
                is_entrance_exit = settings.is_entrance_camera(session.last_camera_id)
                if is_entrance_exit and session.status == "exited":
                    session.exit_type = "CHECK_OUT"
                    session.presence_status = "OUT"
                else:
                    session.exit_type = "VIDEO_ENDED"
                    session.presence_status = "VIDEO_ENDED"
                
                session.status = "exited"
                session.last_seen = actual_exit_time
                # Write the attendance record JSON for the known employee
                self.attendance_manager.generate_attendance_record(session)
        
        # Write final daily summary text and JSON reports
        date_str = ts.strftime('%Y%m%d')
        self.attendance_manager.generate_daily_summary_report(date_str)
