...
class Renderer:
    ...
    def draw(
        ...
        for s in sessions:
            ...
            # Compute total cumulative working duration (resumes seamlessly from prior visits)
            duration_sec = getattr(s, "working_duration", (s.last_seen - s.first_seen).total_seconds())
            ...
            presence = getattr(s, "presence_status", "IN")
            presence_tag = f" [{presence}]"
            lines = [
                f"Employee: {s.employee_name}{presence_tag}",
                f"ID: {s.employee_id} | Status: {presence}",
                f"Track: {s.track_id} | Match: {getattr(s, 'recognition_confidence', 0.0):.1f}%",
                f"Head Dir: {head_dir}",
                f"Work Timer: {session_time}",
