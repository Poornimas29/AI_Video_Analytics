...
class FastReIDEngine:
    ...
    @property
    def use_deep_model(self) -> bool:
        return self._backend in ("osnet", "mobilenet")

    @use_deep_model.setter
    def use_deep_model(self, value: bool) -> None:
        if not value:
            self._backend = "histogram"
        elif self._backend == "histogram":
    ...
    def compute_similarity(
        ...
        f1 = np.asarray(feat1, dtype=np.float32).flatten()
        f2 = np.asarray(feat2, dtype=np.float32).flatten()

        if f1.shape != f2.shape or len(f1) == 0:
            return 0.0
