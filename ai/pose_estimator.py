...
class MediaPipePoseEstimator:
    ...
    def __init__(self) -> None:
        ...
        if use_mediapipe_tasks:
            ...
            is_mocked = type(getattr(vision.PoseLandmarker, "create_from_options", None)).__name__ in ("MagicMock", "Mock")

            if not os.path.exists(model_path):
                logger.warning("Model file '%s' not found. Using Heuristic fallback.", model_path)
                self.use_fallback = True
            elif sys.platform == "darwin" and not is_mocked:
                # Upstream MediaPipe C++ library crashes on macOS ARM64 in DrishtiMetalHelper
                logger.info("macOS runtime detected — using stable Heuristic Pose fallback.")
                self.use_fallback = True
            else:
                try:
